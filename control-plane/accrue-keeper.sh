#!/bin/bash
# Accrue keeper — periodically calls accrueAll() on BorrowFeeEngine
# and accrueFunding() on FundingRateEngine for all 20 active markets.

DEPLOY_ENV=/home/lever/lever-protocol/control-plane/deploy-env.sh
export PATH="/home/lever/.foundry/bin:$PATH"

echo "[$(date)] Accrue keeper started (20 markets)"

while true; do
  source "$DEPLOY_ENV"

  # Borrow fee accrual (single call covers all markets)
  RESULT=$(cast send $BORROW_FEE_ENGINE "accrueAll()" \
    --private-key $PRIVATE_KEY --rpc-url $RPC_URL --gas-limit 2000000 2>&1)
  if echo "$RESULT" | grep -q "status.*1"; then
    echo "[$(date)] accrueAll() success"
  else
    ERR=$(echo "$RESULT" | grep -E 'Error|error' | head -1)
    echo "[$(date)] accrueAll() failed: $ERR"
  fi

  # Funding rate accrual (per-market) — all 20 markets
  FUNDED=0
  for MKT in \
    ...
    0x7155116cef46226d9a58e096c87fba03555313c85b9b9b649dca754090845136; do
    RESULT=$(cast send $FUNDING_RATE_ENGINE "accrueFunding(bytes32)" "$MKT" \
      --private-key $PRIVATE_KEY --rpc-url $RPC_URL --gas-limit 300000 2>&1)
    if echo "$RESULT" | grep -q "status.*1"; then
      FUNDED=$((FUNDED + 1))
    fi
  done
  echo "[$(date)] Funding accrued for $FUNDED/20 markets"
