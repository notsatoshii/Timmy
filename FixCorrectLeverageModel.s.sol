...
/// @title Fix the Correct LeverageModel that ExecutionEngine is using
/// @notice Fix risk parameters on 0xA7D95F94DA06E29FC8EFF948BCA3B4AF1D2585ED
contract FixCorrectLeverageModel is Script {

    function run() external {
        vm.startBroadcast();

        // This is the LeverageModel address that ExecutionEngine is actually pointing to
        ILeverageModel leverageModel = ILeverageModel(0xA7D95F94DA06E29FC8EFF948BCA3B4AF1D2585ED);
        ...
        console2.log("=== Fixing CORRECT LeverageModel Risk Parameters ===");
        console2.log("LeverageModel address:", address(leverageModel));

        // Set very low volatility and high depth threshold to maximize leverage
        uint256 sigmaBaseline = 0.10e18;  // 10% baseline volatility (very low)
        uint256 depthThreshold = 100e18;  // 100 USDT (high depth threshold)
        ...
        console2.log("SpaceX new max leverage:", newMax / 1e18);
        ...
        console2.log("SpaceX final max leverage:", spacexFinal / 1e18);

        if (spacexFinal >= 12e18) {
            console2.log("SUCCESS: Max leverage >= 12x for SpaceX");
        } else if (spacexFinal >= 10e18) {
            console2.log("GOOD: Max leverage >= 10x for SpaceX");
        } else {
            console2.log("STILL ISSUES: Max leverage < 10x for SpaceX");
        }
