...
/// @title LeverageModel
/// @notice Three-step pipeline computing Effective_Max_Leverage for any market at any time.
///         Step 1: Platform Ceiling (TVL × IFR × utilization).
///         Step 2: Compress by R_adjusted.
///         Step 3: Compress AGAIN by M_market (intentional double application — WP Section 9.9).
/// @dev M_market is applied TWICE for leverage:
...
contract LeverageModel is ILeverageModel, AccessControl, Pausable {
    ...
    function validateLeverage(bytes32 marketId, uint256 leverage) external view override returns (bool valid) {
        if (leverage < MIN_LEVERAGE) revert LeverageModel__LeverageBelowMinimum(leverage);
