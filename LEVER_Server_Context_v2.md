...
### NOT Working — Requires Contract Redeployment

**Bug 1: Vault → RewardsDistributor mismatch**
- LeverVault points to wrong RD (`0x77F8`, empty) instead of correct one (`0xab8D`, has funds)
- `leverVault` is immutable — must redeploy: LeverVault + ExecutionEngine + SettlementEngine
- DO NOT attempt frontend hacks. Requires contract redeployment.

**Bug 2: LiquidationEngine — Constructor args were SCRAMBLED at original deploy**
- `positionManager()` returns OracleAdapter address
- `leverVault()` returns InsuranceFund address  
- `executionEngine()` returns PositionManager address
- Must redeploy with correct arg order

**See:** `LEVER_Redeployment_Handoff_v2.md` for full zero-placeholder redeployment plan.

### Known Frontend Bugs
1. Vault utilization may still show 0.0%
2. Wallet balance may show wrong decimals
3. No "Claim Rewards" or "Compound" button on vault page (blocked until redeployment)
...
### ❌ Needs Redeployment (addresses will change after redeploy)
```
LeverVault:          0x84a1Eb3b1eFD60b193b271DCfaB2711cE1c41921  ← WRONG RD
ExecutionEngine:     0xc749C6aAe8a5ACBDD924DF7f833Dd3115307a60D  ← References wrong vault
SettlementEngine:    0x9c7E9496A25Bf06f163A4483e5702ac350e8e9aD  ← References wrong vault
LiquidationEngine:   0x2A42Ef441CAbF34D3Ff9B9867CAf4Ae087FEC42E  ← Scrambled constructor args
```
